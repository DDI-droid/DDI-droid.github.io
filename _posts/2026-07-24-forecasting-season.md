...
Watching the bot work is its own reward. In one captured run it spent seven minutes on a Slovenian parliamentary seat-redistribution question, building Wilson intervals over local polls, modeling D'Hondt seat allocation, and rewriting its search queries in Slovene with site filters for Ljubljana pollsters. The bot had opinions about Slovenian coalition politics. We mostly watched.

## ProphetHacks, or 32 hours and a bot called GreenieBot

Before writing any code for the hackathon, we did something that felt almost like cheating and is actually just forecasting. Metaculus bots post their reasoning publicly, and we had been reading the traces of a strong one called GreenieBot. So we reverse-engineered it from its own words. We pulled its public traces, distilled them into two lists, behaviors to copy and behaviors to avoid, and baked both into our prompts as few-shot examples. Some of those lessons are still in our prompt files verbatim, like the reminder that models systematically underprice questions where a benchmark score is already near the threshold, and the line we quote at each other: overweighting formal caution can be as damaging as overconfidence.
